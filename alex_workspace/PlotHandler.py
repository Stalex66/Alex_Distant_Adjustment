...
def plot3D_stars(labels: np.array, df: pd.DataFrame, filename: str, output_pathname: str = None, return_fig: bool = False):
    """
    Function for creating a 3D plot of star clusters.
    """
    cs = labels
    df_plot = df.loc[cs != -1]  # remove field stars from dataframe
    clustering_solution = cs[cs != -1].astype(int)  # remove field stars from label array and convert to int
    cut_us = np.random.uniform(0, 1, size=clustering_solution.shape[0]) < 0.1  # background cu
    # plotting specs
    bg_opacity = 0.1
    bg_color = 'gray'
    ...
    axis_kwargs = dict(
        showbackground=False, showline=False, zeroline=True, zerolinecolor='grey', zerolinewidth=2,
    ...
    # Finalize layout
    fig.update_layout(
        title="3D Star Clusters",
        scene=dict(
            xaxis=dict(title='X [pc]', **axis_kwargs),
            yaxis=dict(title='Y [pc]', **axis_kwargs),
            zaxis=dict(title='Z [pc]', **axis_kwargs)
        ),
        showlegend=True,
...
class PlotHandler:
    ...
    def plot_labels_3DOnly(self,labels, title = ""):
        """
        Plots the labels in a 3D space.
        ...
        """
        # Plot the clusters
        fig = plot3D(labels, self.df_region, title, self.output_path, hrd= False, return_fig=True)
        fig.show()
